...
;(function () {
  ...
  // You can override the endpoint by specifying data-api on the <script>
  const API_URL     = SCRIPT_TAG.getAttribute("data-api") ||
                      "http://18.133.103.39/ask";
  // Here is the critical fix: read the data-key attribute
  ...
  if (!API_KEY) {
    console.error("FAQ Widget: no data-key attribute found. Please add data-key=“Your_API_Key” to the <script> tag.");
    return;
  }
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        headers: {
          ...
          // ← include our API key in every request
          "X-API-Key": API_KEY
        },
        body: JSON.stringify({ question: q })
      });

      // If you want more robust error‐handling, check res.ok first:
      // if (!res.ok) { replaceTyping("Server error."); return; }

      const data = await res.json();
      replaceTyping(data.answer || "Sorry, I’m not sure.");
    } catch(e) {
      replaceTyping("Error contacting server.");
      console.error("FAQ Widget fetch error:", e);
    }
  }
